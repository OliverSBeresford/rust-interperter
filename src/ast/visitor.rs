...
pub trait VisitorMutable<T> {
    // Expression visitor methods
    fn visit_literal(&mut self, value: &mut Token) -> T;
    fn visit_grouping(&mut self, expression: &mut Expr) -> T;
    fn visit_unary(&mut self, operator: &mut Token, right: &mut Expr) -> T;
    fn visit_binary(&mut self, left: &mut Expr, operator: &mut Token, right: &mut Expr) -> T;
    fn visit_variable(&mut self, name: &mut Token, depth: &mut Depth) -> T;
    fn visit_assign(&mut self, name: &mut Token, value: &mut Expr, depth: &mut Depth) -> T;
    fn visit_logical_or(&mut self, left: &mut Expr, right: &mut Expr) -> T;
    fn visit_logical_and(&mut self, left: &mut Expr, right: &mut Expr) -> T;
    ...
    fn visit_var_statement(&mut self, name: &mut Token, initializer: &mut Option<Expr>) -> T;
    fn visit_block_statement(&mut self, statements: &mut [Statement]) -> T;
    fn visit_if_statement(&mut self, condition: &mut Expr, then_branch: &mut Statement, else_branch: &mut Option<Box<Statement>>) -> T;
    fn visit_while_statement(&mut self, condition: &mut Expr, body: &mut Statement) -> T;
    fn visit_function_statement(&mut self, statement: &mut Statement) -> T;
    fn visit_return_statement(&mut self, keyword: &mut Token, value: &mut Option<Expr>) -> T;
    fn visit_class_statement(&mut self, name: &mut Token, methods: &mut [Statement]) -> T;

    fn visit_expression(&mut self, expr: &mut Expr) -> T {
        match expr {
            Expr::Literal { value } => self.visit_literal(value),
            Expr::Grouping { expression } => self.visit_grouping(expression),
            Expr::Unary { operator, right } => self.visit_unary(operator, right),
